...
if __name__ == "__main__":
	...
	if len(sys.argv[0:]) == 1:
		...
		print('\t<outFsID> ID for the output (target) sample rate (defaults to 1)')
		print('\t<inFsID>  ID for the input (source) sample rate (defaults to 0)\n')
		print('Sample rate IDs are:')
		print('\t 0 - 2.4   Msamples/sec (default input sample rate)')
		print('\t 1 - 2.88  Msamples/sec (default output sample rate)')
